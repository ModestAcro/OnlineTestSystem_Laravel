...
<main class="main my-5">
    <div class="container card shadow p-4">
        <form action="{{route('questionMulti.update', $question->id)}}" method="post">
            @csrf
            @method('patch')
                ...
                @foreach($question->answers as $key => $answer)
                    <div class="answer-options d-flex flex-wrap align-items-center gap-3 mt-3 p-2 border rounded shadow-sm">
                        <div class="flex-grow-1">
                            <label class="form-label">Odpowiedź: {{$key + 1}}:</label>
                            <textarea type="text" class="form-control" name="answers[{{$key}}][text]" required>{{ $answer->title }}</textarea>
                        </div>
                        <div class="col-auto">
                            <label class="form-label">Punkty</label>
                            <input type="number" class="form-control" name="answers[{{$key}}][points]" value="{{ $answer->points }}" required>
                        </div>
                        <div class="col-auto d-flex align-items-center">
                            <div class="form-check mt-3">
                                <input type="checkbox" class="form-check-input" name="answers[{{$key}}][is_correct]" value="1" {{ $answer->is_correct ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-between align-items-center mt-2 ">
                <button class="btn btn-outline-danger mt-3" id="add-answer-btn">
            ...
            <div class="d-flex flex-row bd-highlight mb-3 gap-2">
                ...
                <a href="{{ route('questionMulti.show', $question->id) }}" class="btn btn-outline-danger mt-4 mb-5">Anuluj</a>
                <div class="ms-auto">
                    <a class="btn btn-outline-danger mt-4" data-bs-toggle="modal" data-bs-target="#deleteModal">Usuń</a>
                </div>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{route('questionMulti.delete', $question->id)}}" method="post">
                        @csrf
                        @method('delete')
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('add-answer-btn').addEventListener('click', function () {
            ...
            <div class="col-auto">
                <label class="form-label">Punkty</label>
                <input type="number" class="form-control" name="answers[${answerIndex}][points]" value="0" required>
            </div>
            <div class="col-auto d-flex align-items-center">
                <div class="form-check mt-3">
