...
<main class="main my-5">
    <div class="container card shadow p-4">
        <form action="{{route('groups.update', $group->id)}}" method="post">
            ...
            <h1 class="mt-2">Rok</h1>
            <input class="form-control mb-3" rows="4" name="year" value="{{$group->year}}">

            <h1 class="mt-2">Nazwa</h1>
            <input class="form-control mb-3" rows="4" name="name" value="{{$group->name}}">

            <h1 class="mt-4">Kierunek</h1>
            <select name="course" class="form-select mb-4" required>
                @foreach($courses as $course)
                    <option value="{{ $course->id }}"
                            @if($course->id == old('course', $group->course_id)) selected @endif>
                        {{ $course->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="subject" class="form-select mb-4" required>
                @foreach($subjects as $subject)
                    <option value="{{ $subject->id }}"
                            @if($subject->id == old('subject', $group->subject_id)) selected @endif>
                        {{ $subject->name }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="d-flex flex-row bd-highlight mb-3 gap-2">
                ...
                <a href="{{ route('groups.show', $group->id) }}" class="btn btn-outline-danger mt-4 mb-5">Anuluj</a>
                <div class="ms-auto">
